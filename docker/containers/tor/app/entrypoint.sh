...
# Allow browser to run under user
chown -R $VNC_USER:$VNC_USER /opt/tor-browser

# Start fluxbox and app
su $VNC_USER -c "export DISPLAY=\":0\" && fluxbox 2> /dev/null &"
su $VNC_USER -c "export DISPLAY=\":0\" && /opt/tor-browser/Browser/start-tor-browser -n0 xterm 2> /dev/null"
#export DISPLAY=":0" && watch -n0 xterm 2>/dev/null
